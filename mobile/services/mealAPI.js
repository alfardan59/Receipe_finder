...
export const MealAPI = {
  ...
  getCategories: async () => {
    try {
      ...
      // Remove Beef
      return allCategories.filter((cat) => cat.strCategory !== "Beef" && cat.strCategory !== "Pork");
    } catch (error) {
  ...
  filterByCategory: async (category) => {
    if (category === "Beef" || category === "Pork") {
      console.warn("Beef category is disabled in this app");
      return []; // or throw new Error("Category not available") if you prefer
    }
    try {
      const response = await fetch(
